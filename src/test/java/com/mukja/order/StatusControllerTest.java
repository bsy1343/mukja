...
class StatusControllerTest {
    ...
    @BeforeEach
    void seed() {
        repo.reset("kt", "stat-team");
        service.submit("kt", "stat-team", "백상열",
            java.util.List.of(new OrderService.LineInput(102, java.util.Map.of("temp", "ice"))));
    }

    @AfterEach
    void clean() { repo.reset("kt", "stat-team"); }

    @Test
    void statusPageShowsStats() throws Exception {
        mvc.perform(get("/coffee/kt/stat-team/status")).andExpect(status().isOk())
           .andExpect(content().string(containsString("아메리카노")));
    }

    @Test
    void summaryTxtIsPlainText() throws Exception {
        mvc.perform(get("/coffee/kt/stat-team/status/summary.txt")).andExpect(status().isOk())
    ...
    void statusFragmentForHtmxRequest() throws Exception {
        mvc.perform(get("/coffee/kt/stat-team/status").header("HX-Request", "true"))
