...
사내 커피·점심 주문을 **가게 × 팀 단위 상시 주문판**으로 취합하는 모바일 우선 웹앱.
보드가 곧 **팀별 일일 세션** 역할을 한다 — 당번이 마감 시각을 정해 그 이후 주문을 막고, 매일 자정(KST)에 자동 초기화돼 다음 날 다시 사용된다. (별도의 세션 라이프사이클 객체는 없고 보드 자체가 그 역할을 한다.)
